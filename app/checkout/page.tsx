    ...
    try {
      ...
      if (paymentMethod === 'wave') {
        ...
        if (!omRes.ok) {
          throw new Error(
            omData?.error
              ? `Orange Money : ${omData.error}`
              : 'Erreur Orange Money. Vérifiez votre numéro et réessayez.'
          )
        }
        router.push(`/order/${orderId}/success`)
      } else {
        // WhatsApp — notify admin + customer then redirect
        await fetch('/api/payment/whatsapp/notify', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ orderId }),
        })
            ...
            {step === 'payment' && (
              <div className="bg-white rounded-2xl p-6 shadow-sm">
                ...
                <div className="space-y-3 mb-6">
                  ...
                  <label className={`flex items-center gap-4 p-4 rounded-xl border-2 cursor-pointer transition-colors ${paymentMethod === 'whatsapp' ? 'border-green-400 bg-green-50' : 'border-gray-100 hover:border-green-200 hover:bg-green-50/50'}`}>
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">Commandez et payez via WhatsApp</p>
                    </div>
                    {paymentMethod === 'whatsapp' && <span className="ml-auto text-green-500">✓</span>}
                  </label>
                </div>
