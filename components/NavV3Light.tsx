...
// Rose foncé logo : #9e3d58
const ACCENT = '#9e3d58'

export default function NavV3Light() {
  ...
  return (
    <>
      <nav
        ...
        style={{ width: scrolled ? '92%' : '80%', maxWidth: '900px' }}
      >
        <div
          className="flex items-center justify-between px-5 py-3 rounded-2xl transition-all duration-500"
          style={{
            background: scrolled ? 'rgba(253,250,247,0.95)' : 'rgba(253,250,247,0.75)',
            backdropFilter: 'blur(24px)',
            border: `1px solid rgba(158,61,88,${scrolled ? '0.15' : '0.08'})`,
            boxShadow: scrolled ? '0 8px 40px rgba(158,61,88,0.08)' : 'none',
          }}
        >
          {/* Logo */}
          <Link href="/v3-light" className="flex items-center gap-2.5 group flex-shrink-0">
            <div className="relative w-7 h-7 rounded-full overflow-hidden"
              style={{ boxShadow: `0 0 0 1px rgba(158,61,88,0.3)` }}>
              <Image src="/images/logo/logo.png" alt="Makiné" fill className="object-cover" />
            </div>
            <span className="font-serif text-sm font-bold tracking-wide" style={{ color: '#1a0a12' }}>
              Makiné
            </span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-1">
            {[
              ...
              <Link key={href} href={href}
                ...
                onMouseEnter={e => (e.currentTarget.style.color = '#1a0a12')}
                onMouseLeave={e => (e.currentTarget.style.color = 'rgba(26,10,18,0.5)')}
              >{label}</Link>
            ))}
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/checkout"
              ...
              style={{ background: `rgba(158,61,88,0.08)`, color: '#9e3d58' }}>
        ...
        {/* Mobile drawer */}
        {open && (
          <div className="mt-2 rounded-2xl px-4 py-3 flex flex-col gap-1"
            style={{ background: 'rgba(253,250,247,0.97)', backdropFilter: 'blur(24px)', border: `1px solid rgba(158,61,88,0.12)` }}>
            {[
              { href: '/v3/boutique', label: 'Boutique' },
              ...
              <Link key={href} href={href} onClick={() => setOpen(false)}
                ...
                style={{ color: 'rgba(26,10,18,0.6)', borderColor: 'rgba(158,61,88,0.08)' }}>
                {label}
              </Link>
            ))}
            <a href="https://wa.me/221710581711" target="_blank" rel="noopener noreferrer"
              className="mt-1 py-2.5 text-sm" style={{ color: '#4ade80' }}>
